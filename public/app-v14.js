...
state.groups=grouped.groups.map(g=>({
  ...
  selected:0
})).filter(g=>g.images.length&&g.captions.length);
    ...
    if(storyResponse&&Array.isArray(storyResponse.stories)){
      state.storyIdeas=storyResponse.stories.map((story,index)=>{
        ...
        return{
          ...
          captions:Array.isArray(story.text_options)?story.text_options:[],
          selected:0
        };
...
function storyCaptionOptions(title,purpose,accountName){
  ...
  return [
    {
      label:'Quick update',
      text:`A little update from ${name||'us'} — ${subject}.`
    },
    {
      label:'Engagement',
      text:`What do you think of ${subject.toLowerCase()}?`
    },
    {
