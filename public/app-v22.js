...
function renderProfileReference(){
  ...
  const handle=platform==='TikTok'?ctx.tiktok:platform==='LinkedIn'?ctx.linkedin:ctx.instagram;

  if($('profilePlatformLabel'))$('profilePlatformLabel').textContent=platform;
  if($('profileDisplayName'))$('profileDisplayName').textContent=ctx.name||'Your account name';
  if($('profileHandle'))$('profileHandle').textContent=normaliseHandle(handle);
  if($('profileBio'))$('profileBio').textContent=buildProfileBio(ctx);
  if($('profileWebsite'))$('profileWebsite').textContent=domainLabel(ctx.website);

  // Do not invent profile statistics before a real account connection.
  if($('profilePostsCount'))$('profilePostsCount').textContent='—';
  if($('profileFollowersCount'))$('profileFollowersCount').textContent='—';
  if($('profileFollowingCount'))$('profileFollowingCount').textContent='—';

  // Use a neutral PicPlanr placeholder until the real profile image is available.
  const logo=document.querySelector('.brand-logo img');
  if(logo&&$('profileAvatar')){
    $('profileAvatar').src=logo.src;
    $('profileAvatar').classList.add('preview-placeholder');
  }

  const profileImages=state.images.filter(image=>image.localImage).slice(0,6);
  document.querySelectorAll('.profile-grid-tile').forEach((tile,index)=>{
    ...
    if(image){
      ...
      tile.innerHTML='<span>Connect account</span>';
    }
  });

  if($('profilePreviewBadge'))$('profilePreviewBadge').textContent='Preview only';
}
...
$('buildProfile').onclick=async()=>{const btn=$('buildProfile');showLoading('Analysing your account','PicPlanr is learning the account type, goals and communication style.',btn);try{state.profile=(await api('profile',{context:context()})).profile;renderProfile();$('statusPill').textContent='Voice profile ready';renderProfileReference();}catch(e){alert(e.message)}finally{hideLoading(btn)}};
