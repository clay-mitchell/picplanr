...
$('buildProfile').onclick=async()=>{const btn=$('buildProfile');showLoading('Analysing your account','PicPlanr is learning the account type, goals and communication style.',btn);try{state.profile=(await api('profile',{context:context()})).profile;renderProfile();$('statusPill').textContent='Voice profile ready';}catch(e){alert(e.message)}finally{hideLoading(btn)}};
...
for(let groupIndex=0;groupIndex<draftGroups.length;groupIndex++){
  ...
  if(validation&&validation.group&&validation.group.approved_for_display!==false){
    ...
    state.groups.push({
      ...
      image_ids:draft.image_ids,
      images:draft.images,
      captions:Array.isArray(fixed.captions)?fixed.captions:[],
