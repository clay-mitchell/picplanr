...
function monthWeeks(base){
  const first=new Date(base.getFullYear(),base.getMonth(),1);
  const last=new Date(base.getFullYear(),base.getMonth()+1,0);
  const weeks=[];
  let cursor=new Date(first);
  cursor.setDate(first.getDate()-((first.getDay()+6)%7)); // Monday
  while(cursor<=last){
    const start=new Date(cursor),end=new Date(cursor);end.setDate(end.getDate()+6);
    weeks.push({start,end,count:0});
    cursor.setDate(cursor.getDate()+7);
  }
  return weeks;
}
function dateForPreferredDay(week,dayName,usedDates){
  ...
  for(const offset of candidateOrder){
    ...
    if(d.getMonth()!==calendarDate.getMonth())continue;
    if(!usedDates.has(d.toDateString())){usedDates.add(d.toDateString());return d}
  }
  const fallback=new Date(week.start);fallback.setDate(fallback.getDate()+preferred);
  usedDates.add(fallback.toDateString());
  return fallback;
}
function buildScheduledPosts(){
  ...
  const weeks=monthWeeks(calendarDate).filter(w=>{
    const monthStart=new Date(calendarDate.getFullYear(),calendarDate.getMonth(),1);
    const monthEnd=new Date(calendarDate.getFullYear(),calendarDate.getMonth()+1,0,23,59,59);
    return w.end>=monthStart&&w.start<=monthEnd;
  });
  const usedDates=new Set();
  const total=approved.length;
  const targetPerWeek=Math.min(4,Math.max(1,Math.ceil(total/weeks.length)));
  let weekCursor=0;

  scheduledPosts=approved.map(({group,groupIndex},postIndex)=>{
    let selectedWeek=null;
    for(let tries=0;tries<weeks.length;tries++){
      const candidate=weeks[(weekCursor+tries)%weeks.length];
      if(candidate.count<4 && candidate.count<targetPerWeek){
    ...
    if(!selectedWeek){
      selectedWeek=weeks
        .filter(w=>w.count<4)
        .sort((a,b)=>a.count-b.count)[0]||weeks[postIndex%weeks.length];
    }
    selectedWeek.count++;

    const date=dateForPreferredDay(selectedWeek,group.schedule.day,usedDates);
    ...
    return{
      ...
      objective:group.objective,reason:`Spread across the month with a maximum of four posts per week. ${group.schedule.reason}`,
      platform:context().platform,image:group.images[0]?.localImage||'',
      caption:selected?.text||'',date,publishStatus:'Draft'
    }
  }).sort((a,b)=>a.date-b.date);

  const weeklyCounts={};
  scheduledPosts.forEach(p=>{
    const start=startOfWeek(p.date).toISOString().slice(0,10);
    weeklyCounts[start]=(weeklyCounts[start]||0)+1;
  });
  ...
  $('calendarSummary').textContent=`${scheduledPosts.length} approved posts spread across ${Object.keys(weeklyCounts).length} weeks. Maximum 4 posts per week.`;
  return true
}
