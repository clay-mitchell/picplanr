const $=id=>document.getElementById(id);const state={accountType:'Business',profile:null,files:[],images:[],groups:[],approved:new Set(),storyIdeas:[],approvedStories:new Set()};
...
$('analyseFolder').onclick=async()=>{if(!state.profile){alert('Please create the account voice profile first.');show('onboarding');return}const total=state.files.length;$('progressWrap').classList.remove('hidden');state.images=[];state.storyIdeas=[];state.approvedStories.clear();try{for(let i=0;i<total;i++){$('progressText').textContent=`Analysing ${state.files[i].name}`;$('progressCount').textContent=`${i+1} / ${total}`;$('progressBar').style.width=`${Math.round(i/total*100)}%`;const compressed=await compress(state.files[i]);const out=await api('image',{context:context(),profile:state.profile,image:{name:state.files[i].name,dataUrl:compressed.dataUrl}});state.images.push({...out.analysis,localImage:compressed.dataUrl,filename:state.files[i].name,index:i,width:compressed.width,height:compressed.height,orientation:compressed.orientation})}$('progressText').textContent='Grouping related images and writing captions…';const grouped=await api('group',{context:context(),profile:state.profile,images:state.images.map(({localImage,...x})=>x)});if(!grouped||!Array.isArray(grouped.groups))throw new Error('PicPlanr could not create content ideas from this upload. Please try again with fewer images.');
state.groups=grouped.groups.map(g=>({
  ...g,
  image_indexes:Array.isArray(g.image_indexes)?g.image_indexes:[],
  captions:Array.isArray(g.captions)?g.captions:[],
  schedule:g.schedule||{day:'Tuesday',time:'6:00 PM',reason:'Balanced content timing'},
  images:(Array.isArray(g.image_indexes)?g.image_indexes:[]).map(i=>state.images[i]).filter(Boolean),
  selected:0
})).filter(g=>g.images.length&&g.captions.length);$('progressBar').style.width='100%';renderGroups();show('posts');$('statusPill').textContent=`${state.groups.length} post groups created`}catch(e){alert(e.message)}};
...
function storyCaptionOptions(title,purpose){
  return [
    {
      label:'Natural',
      text:`A quick look at ${title.toLowerCase()}.`
    },
    {
      label:'Engagement',
      text:`What would you like to see more of from ${title.toLowerCase()}?`
    },
    {
      label:'Goal-led',
      text:purpose==='Encourage replies'
        ?`Send us your thoughts on ${title.toLowerCase()}.`
        :`See more from ${title.toLowerCase()} and follow along.`
    }
  ];
}
...
function fallbackStoryIdeas(){
  ...
  portraitImages.forEach((image,index)=>{
    ...
    const title=relatedGroup?.title||image.primary_subject||'Portrait content';
    const time=relatedGroup?.schedule?.time||'Flexible';

    ideas.push({
      id:`story-${image.index}-behind`,
      title:`Behind the scenes: ${title}`,
      best_time:time,
      purpose:'Build familiarity',
      frames:[
        `Open with the portrait image from “${title}”`,
        'Add one short fact or observation about the moment',
        'Finish with a simple reaction slider'
      ],
      image:image.localImage,
      imageIndex:image.index,
      captions:storyCaptionOptions(title,'Build familiarity'),
      selected:0
    });

    ideas.push({
      id:`story-${image.index}-question`,
      title:`Audience question: ${title}`,
      best_time:time,
      purpose:'Encourage replies',
      frames:[
        `Use the portrait image from “${title}”`,
        'Ask one clear either-or question',
        'Share the response or result later'
      ],
      image:image.localImage,
      imageIndex:image.index,
      captions:storyCaptionOptions(title,'Encourage replies'),
      selected:0
    });
  });
  ...
  $('storyIdeas').innerHTML=ideas.map((idea,index)=>`
    <article class="story-review-card">
      ...
      <button class="primary approve-story" data-story="${index}">
        ${state.approvedStories.has(idea.id)?'Approved for scheduling':'Approve selected Story text'}
      </button>
    </article>`).join('');
