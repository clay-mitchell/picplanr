...
function fallbackStoryIdeas(){
  ...
  state.groups.forEach((g,index)=>{
    ...
    const title=g.title||'Content idea';
    const time=g.schedule?.time||'Flexible';
...
function renderStoryIdeas(){
  let ideas=[];
  state.groups.forEach(g=>(g.story_ideas||[]).forEach(s=>ideas.push(s)));
  if(!ideas.length)ideas=fallbackStoryIdeas();
  ideas=ideas.slice(0,9);

  $('storyCount').textContent=`${ideas.length} idea${ideas.length===1?'':'s'}`;
  $('storyIdeas').innerHTML=ideas.map((idea,index)=>`
    <article class="story-idea-card">
      ${idea.image?`<img class="story-idea-image" src="${idea.image}" alt="">`:''}
