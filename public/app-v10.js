...
function fallbackStoryIdeas(){
  return state.groups.slice(0,6).map((g,index)=>({
    title:index%2===0?`${g.title}: behind the scenes`:'Quick audience question',
    best_time:g.schedule?.time||'Flexible',
    purpose:index%2===0?'Build familiarity':'Encourage replies',
    frames:index%2===0
      ?[
        `Open with the strongest image from “${g.title}”`,
        'Add one short behind-the-scenes detail',
        'Finish with a simple reaction slider or question'
      ]
      :[
        `Use one image from “${g.title}”`,
        'Ask followers for their preference or opinion',
        'Share the result or response later that day'
      ]
  }));
}
