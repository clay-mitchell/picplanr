...
function makeFallbackGroupForImage(image){
  const subject=image.primary_subject||image.literal_summary||image.confirmed_subjects?.[0]?.name||'Uploaded image';
  const safeSubject=cleanCaptionText(subject).replace(/\.$/,'')||'Uploaded image';
  return {
    title:safeSubject,
    image_ids:[image.image_id],
    format:'Single post',
    objective:'Keep this uploaded image available as a content idea.',
    group_reason:'This image was kept as its own post idea so every uploaded photo remains available to review.',
    schedule:{day:'Tuesday',time:'11:00',reason:'Balanced weekday posting time.'},
    captions:normaliseCaptions([
      {label:'Natural',text:`A closer look at ${safeSubject.toLowerCase()}.`},
      {label:'Engagement',text:`What do you think of this ${safeSubject.toLowerCase()}?`},
      {label:'Goal-led',text:'Message us to find out more.'}
    ]),
    images:[image],
    selected:0,
    accuracy_status:'checked',
    accuracy_notes:['Kept from upload because every photo should remain available in the workflow.']
  };
}

function ensureEveryUploadedImageIsUsed(){
  const used=new Set();
  state.groups.forEach(group=>(group.images||[]).forEach(image=>used.add(image.index)));
  const missing=state.images.filter(image=>!used.has(image.index));
  missing.forEach(image=>state.groups.push(makeFallbackGroupForImage(image)));
}
...
ensureEveryUploadedImageIsUsed();
state.groups.sort((a,b)=>{
  const aIndex=a.images?.[0]?.index??9999;
  const bIndex=b.images?.[0]?.index??9999;
...
function renderGroups(){
      ...
      <div class="caption-options">
        ${normaliseCaptions(g.captions).map((c,ci)=>`
          <div class="caption-option ${ci===g.selected?'selected':''}" data-g="${gi}" data-c="${ci}">
            ...
            <span class="edit-caption-label">Editable caption</span>
            <textarea class="caption-edit-area post-caption-edit" data-g="${gi}" data-c="${ci}">${esc(c.text)}</textarea>
          </div>`).join('')}
      </div>
  ...
  document.querySelectorAll('.caption-option').forEach(x=>x.onclick=(event)=>{
    if(event.target&&event.target.classList.contains('caption-edit-area'))return;
    state.groups[+x.dataset.g].selected=+x.dataset.c;
    renderGroups();
  });
  document.querySelectorAll('.post-caption-edit').forEach(input=>{
    input.oninput=()=>{
      const groupIndex=+input.dataset.g;
      const captionIndex=+input.dataset.c;
      state.groups[groupIndex].captions[captionIndex].text=input.value;
    };
...
function renderStoryIdeas(){
      ...
      <div class="caption-options story-caption-options">
        ${normaliseStoryCaptions(idea.captions,idea.primary_subject).map((caption,captionIndex)=>`
          <div class="caption-option story-caption-option ${captionIndex===idea.selected?'selected':''}"
            ...
            <span class="edit-caption-label">Editable Story text</span>
            <textarea class="caption-edit-area story-caption-edit" data-story="${index}" data-story-caption="${captionIndex}">${esc(caption.text)}</textarea>
          </div>`).join('')}
      </div>
  ...
  document.querySelectorAll('.story-caption-option').forEach(option=>option.onclick=(event)=>{
    if(event.target&&event.target.classList.contains('caption-edit-area'))return;
  ...
  document.querySelectorAll('.story-caption-edit').forEach(input=>{
    input.oninput=()=>{
      const storyIndex=+input.dataset.story;
      const captionIndex=+input.dataset.storyCaption;
      state.storyIdeas[storyIndex].captions[captionIndex].text=input.value;
    };
    input.onclick=event=>event.stopPropagation();
  });
...
function previewUploadedScreenshot(input,wrapId,imageId){const file=input.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{$(imageId).src=reader.result;$(wrapId).classList.remove('hidden')};reader.readAsDataURL(file)}
...
if($('instagramScreenshotInput'))$('instagramScreenshotInput').onchange=()=>previewUploadedScreenshot($('instagramScreenshotInput'),'instagramScreenshotPreviewWrap','instagramScreenshotPreview');
if($('uploadTikTokScreenshot'))$('uploadTikTokScreenshot').onclick=()=>$('tiktokScreenshotInput').click();
if($('tiktokScreenshotInput'))$('tiktokScreenshotInput').onchange=()=>previewUploadedScreenshot($('tiktokScreenshotInput'),'tiktokScreenshotPreviewWrap','tiktokScreenshotPreview');

function enforceContentIdeasOrder(){
  const posts=document.querySelector('.detailed-ideas');
  const stories=$('storyIdeasPanel');
  if(posts&&stories&&posts.compareDocumentPosition(stories)&Node.DOCUMENT_POSITION_PRECEDING){
    posts.parentNode.insertBefore(posts,stories);
  }
}
const originalShowForContentOrder=show;
show=function(id){
  originalShowForContentOrder(id);
  if(id==='posts')setTimeout(enforceContentIdeasOrder,0);
};
