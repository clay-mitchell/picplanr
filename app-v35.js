...
function show(step){
  ...
  if(!target || !target.classList.contains('view')){
    console.warn('[PicPlanr V35] Unknown page:',step);
    return;
  }
  ...
  const title=$('title');
  if(title)title.textContent=titles[step]||'PicPlanr';
  ...
  if(pill){
    if(step==='audit' && state.audit){
      pill.textContent=`Account strength ${state.audit.overall_score}/100`;
    }else if(step!=='audit'){
      pill.textContent=state.profile?'Workspace ready':'Not started';
    }
  }
...
async function runLiveInstagramAnalysis(){
  ...
  try{
    ...
    show('audit');
    if(document.body.dataset.page==='audit')$('statusPill').textContent=`Account strength ${data.audit.overall_score}/100`;
...
console.info('[PicPlanr] Version 35 Clean loaded');
show(document.querySelector('.nav.active[data-step]')?.dataset.step||'onboarding');
