...
catch(error){console.error(error);let message=error?.message||"Image analysis failed.";
if(error?.status===401)message="The OpenAI API key is invalid or not authorised.";
if(error?.status===429)message="The OpenAI account has reached a rate or billing limit.";
return res.status(error?.status&&error.status<600?error.status:500).json({error:message});}
}