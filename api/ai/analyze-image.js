...
export default async function handler(req, res) {
  ...
  try {
    ...
    await assertAndConsumeUsage({supabase,workspaceId:workspace.id,metric:'image_analyses',quantity:1,metadata:{fileName:fileName||null}});
    ...
    if(error?.statusCode===401)return sendWorkspaceError(res,error,'Sign in to analyse images.');
    if (error.status === 401) {
